...
from e2k_eval.core.utils import (
    ...
    calculate_relative_kana_distance,
)
...
def convert(english: str) -> str:
    if ngram(english):
        ...
        return ngram.as_is(english.lower())
...
def evaluate() -> None:
    ...
    accuracy = sum(
        1
        for item, result in zip(evaluation_data, results, strict=False)
        if item.kana == result
    ) / len(evaluation_data)
    avg_ed = sum(
        calculate_relative_editdistance(item.kana, result)
        for item, result in zip(evaluation_data, results, strict=False)
    ) / len(evaluation_data)
    avg_kana_distance = sum(
        calculate_relative_kana_distance(item.kana, result)
        for item, result in zip(evaluation_data, results, strict=False)
    ) / len(evaluation_data)
    wrong_items = [
        (item.english, item.kana, result)
        for item, result in zip(evaluation_data, results, strict=False)
        if item.kana != result
    ]
